...
```{r}
survey_clean %>%
  ggplot() + geom_density(aes(Income))
```
```{r}
survey_unnest %>%
...
```{r}
remove2 <- survey_clean %>%
  filter(str_detect(Ethnicity, ";"))

survey_clean3 <- survey_clean %>%
  anti_join(remove2)

survey_clean3 %>% 
  ggplot() + geom_bar(aes(x = Ethnicity), stat = "count") + coord_flip()
```
```{r}
class(survey_matched)
```
