...
survey_clean2 <- read_csv(here::here("inputs/data/survey_unnest.csv")) 
```
...
```{r gender, fig.cap="no", echo = FALSE}
...
plot(income_model, 1)
bptest(income_model) #p > 0.05 constant variance
shapiro.test(resid(income_model)) #p > 0.05 means residuals not normally distributed
skewness(survey_matched$Income) #heavily skewed
```

```{r, fig.width=10}
tukey <- transformTukey(survey_matched$Income, plotit = FALSE) #rcompanion
skewness(survey_matched$Income)

income_model_new <- 
  lm((-1*Income^(-0.8)) ~ Gender + Ethnicity + Ethnicity * Gender + EdLevel + DevType + YearsCodePro, 
     data = survey_matched)
summary(income_model_new)
check_model(income_model_new)
shapiro.test(resid(income_model_new)) #p > 0.05 means residuals not normally distributed

income_model_new2 <- 
  lm((-1*Income^(-0.8)) ~ Gender + Ethnicity + Ethnicity * Gender + EdLevel + DevType + YearsCodePro + YearsCodePro * Gender, 
     data = survey_matched)
