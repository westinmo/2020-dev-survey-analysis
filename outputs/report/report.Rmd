...
```{r ethnicity, fig.cap="No", echo = FALSE, fig.height=7}
eth <- survey_clean %>%
  ggplot() + 
...
```{r yearscode, fig.cap="test", fig.height=10, echo = F}
...
```{r devtype1,  fig.width=10, fig.height = 10, fig.cap="test", include=F}
survey_unnest %>%
...
```{r devtype2, fig.cap="test", fig.width=7}
survey_unnest %>%
  ...
  scale_fill_manual(values = c("#FC8D62", "#66C2A5", "#8DA0CB")) +
  theme_light() +
  labs(x = "Number of Respondents", y = "Developer Type", title = "Majority of Developers are Full-Stack, Back-End, and/or Front-End") +
  theme(legend.position = "bottom", plot.title = element_text(face = "bold", size = 10))
```
...
```{r performance, fig.width = 10, fig.height = 10, fig.cap="test", echo=F}
...
```{r coeff, fig.width=10, fig.height = 10, echo = F}
